...
public class Grapher extends JPanel implements Runnable {
			...
			try {
				...
				if (this.nn == null || this.nn.getCurrentNeuralNetwork() == null){
					...
					try {
						...
						if (thisGraph != null)
							thisGraph.drawImage(bi, 0, 0, frame.getWidth(),
				...
				int colorOffset = 1;
				for(int layerId = 0; layerId < this.nn.getCurrentNeuralNetwork().getAI().MAX_LAYERS;layerId++){
					...
					if (layer.getNeuronsPerRow()>0){
						...
						int expectedOffset = (int) (space_layer*(((double)this.nn.getCurrentNeuralNetwork()
								.getAI().getNeuronsInLayer(layerId).size()/layer.getNeuronsPerRow())+2));
				...
				for (Layer layer : this.nn.getCurrentNeuralNetwork()
						.getAI().getLayers()) {
					for (int i = 0; i < layer.neuronsInLayer.size(); i++) {
						...
						graphics.fillRect(
								space_layer * (layer.getLayer() + layeroffset)+3,
								space_neuron * (layer.getNeuronsPerRow()>0?(i % layer.getNeuronsPerRow()):i%100)+height_spacing, 6/2, 6);
						if (n.isTriggered()) {
						...
						graphics.fillRect(
								space_layer * (layer.getLayer() + layeroffset),
								space_neuron * (layer.getNeuronsPerRow()>0?(i %layer.getNeuronsPerRow()):i%100)+height_spacing, 6/2, 6);
						
						if(layer.getLayer()==nn.getCurrentNeuralNetwork().getAI().MAX_LAYERS-1){
							//If they are output neurons:
							if(n instanceof OutputNeuron){
								OutputNeuron on = (OutputNeuron)n;
								if(on.hasName()){
									graphics.setColor(new Color(230,230,230));
									graphics.drawString(on.getName(), space_layer * (layer.getLayer() + layeroffset+1)+3, space_neuron * (layer.getNeuronsPerRow()>0?(i % layer.getNeuronsPerRow()):i%100)+height_spacing+6);
								}
							}
						}
					}
				...
				Graphics2D thisGraph = (Graphics2D) getGraphics();
				if (thisGraph != null)
					thisGraph.drawImage(bi, 0, 0,WIDTH,
							HEIGHT, null);
	 ...
	 * bi.getGraphics(); graphics.fillRect(0, 0, bi.getWidth(), bi.getHeight());
	 * if (view != null) { int tiles = ((view.viewdistance * 2) + 1); int[][]
	 ...
	 * if(n.input.size()==0){ graphics.setColor(new Color(110, 110, 110)); }else
	 * if (n.isTriggered()) { graphics.setColor(new Color(240, 240, 240)); }
	 * else { graphics.setColor(new Color(160 , 160, 160)); } graphics.fillRect(
	 * onWidth, onHeight
	 * 
	 ...
	 * view.universe[w].length; h++) { if (view.universe[w][h] != null) { int r
	 * = 0; int g = 0; int b = 0; if (view.universe[w][h] instanceof Block) g =
	 * 160; if (view.universe[w][h] instanceof Entity) r = 160; if
	 * (tileScanTypes[w][h] == 1) r += 80; if (tileScanTypes[w][h] == 2) g +=
	 * 80;
	 * 
	 * graphics.setColor(new Color(r, g, b)); graphics.fillRect(vsWidth + (w *
	 * tileSize), vsHeight + (h * tileSize), tileSize, tileSize); } } }
	 * graphics.setColor(new Color(0, 0, 0)); for (int Lines = 0; Lines < tiles
	 * + 1; Lines++) { graphics.drawLine(vsWidth + (tileSize * Lines), vsHeight,
	 * vsWidth + (tileSize * Lines), vsHeight + (tileSize * tiles));
	 * graphics.drawLine(vsWidth, vsHeight + (tileSize * Lines), vsWidth +
	 * (tileSize * tiles), vsHeight + (tileSize * Lines)); }
	 * graphics.drawString(
	 ...
	 * { graphics.drawLine( vsWidth + (tileSize / 2) + (((InputNeuron)
	 * inputs).xlink * tileSize), vsHeight + (tileSize / 2) + (((InputNeuron)
	 * inputs).ylink * tileSize), onWidth + (tileSize / 2), onHeight + (tileSize
	 * / 2) + (((OutputMotorNeuron) n).responceid * (tileSize + spacing)));
	 * }else if (inputs instanceof OutputMotorNeuron){ int direction =
	 * (((OutputMotorNeuron) inputs).responceid-((OutputMotorNeuron)
	 * n).responceid); int directionNormalized =
	 * (direction)/(direction<0?-direction:direction); graphics.drawLine(onWidth
	 * + (tileSize / 2), onHeight + (tileSize / 2) + (((OutputMotorNeuron)
	 * inputs).responceid * (tileSize + spacing)), onWidth + (tileSize / 2)
	 * +(direction*(spacing)+(directionNormalized*tileSize)), onHeight +
	 * (tileSize / 2) + (((OutputMotorNeuron) inputs).responceid * (tileSize +
	 * spacing))); graphics.drawLine(onWidth + (tileSize / 2), onHeight +
	 ...
	 * +(direction*(spacing)+(directionNormalized*tileSize)), onHeight +
	 * (tileSize / 2) + (((OutputMotorNeuron) n).responceid * (tileSize +
	 * spacing))); } } } } } Graphics2D thisGraph = (Graphics2D) getGraphics();
	 * if (thisGraph != null) thisGraph.drawImage(bi, 0, 0, 1600, 800, null);
	 * 
